...
| 领域 | 当前状态 | 生产化缺口 |
|------|------|------|
| 基础 Relay | TCP Relay、工作连接、会话桥接、流量统计可用；Relay 已支持共享 token 验证和 mTLS | 仍需限流、配置下发、完整 E2E 和运维监控 |
| QUIC Relay | 服务端 QUIC listener 与 work stream 会话交付已接入，支持 CA 签发证书 | 客户端 QUIC Relay E2E、证书信任链和超时/限流策略仍需补齐 |
| Control Plane | HTTP API：节点注册、心跳、Peer 查询、ACL、密钥、IPAM 接口；支持 Bearer Token + mTLS + SQLite 持久化 + 审计日志 | 尚未实现 gRPC/proto、OIDC |
| P2P/TUN | NAT/STUN/ICE/WireGuard/Mesh 原型有测试覆盖；TUN 平台抽象层支持 Linux/macOS/Windows | 真实 OS TUN 需 root 权限；缺路由下发 |
