...
当前仓库更准确的定位是：**Phase 1 基础 TCP Relay MVP 可用，Phase 2-4 多数能力为原型验证或后端模型，尚未达到生产可用**。

| 领域 | 当前状态 | 生产化缺口 |
|------|------|------|
| 基础 Relay | TCP Relay、工作连接、会话桥接、流量统计可用；Relay 已支持共享 token 验证 | 仍需限流、审计、配置下发、完整 E2E 和运维监控 |
| QUIC Relay | 服务端 QUIC listener 与 work stream 会话交付已接入 | 客户端 QUIC Relay E2E、证书信任链和超时/限流策略仍需补齐 |
| Control Plane | 提供 HTTP API：节点注册、心跳、Peer 查询、ACL、密钥接口；支持 Bearer Token 与 SQLite 持久化 | 尚未实现 gRPC/proto、OIDC/mTLS、ACL 实时下发 |
| P2P/TUN | NAT/STUN/ICE/WireGuard/Mesh 原型有测试覆盖 | `netTun` 是 userspace 测试通道，不是真实系统 TUN；缺 IPAM、路由和平台实现 |
| 智能调度 | scheduler/relay/migration 已强类型接入 P2P Engine | 仍需真实控制 TCP/QUIC/P2P/Relay 路径切换 |
| Dashboard | 后端 API、Vue 管理台、SQLite 持久化启动链路、Release 包部署已接入 | 生产环境仍需 RBAC 细化、HTTPS 反向代理和端到端部署联调 |
...
# Control Plane：生产环境应配置 Bearer Token，后续可替换为 mTLS/OIDC
go run ./cmd/control-plane -listen 127.0.0.1:9090 -api-token <strong-token>

# Dashboard：生产环境必须配置强 secret 和管理员密码
