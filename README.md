...
| Control Plane | 提供最小 HTTP API：节点注册、心跳、Peer 查询、ACL、密钥接口；支持可选 Bearer Token | 尚未实现 gRPC/proto、持久化 Store、OIDC/mTLS、ACL 实时下发 |
...
| Dashboard | 后端 API 具备 bcrypt 密码、可配置 CORS、弱配置启动拒绝 | 当前仍是内存 token/数据存储，缺前端管理台与持久化 |
