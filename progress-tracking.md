...
> **最后更新时间**：2026-06-23（v0.6.0-beta 生产收口：Relay Admin API、Dashboard 客户端监控、桌面服务端实例检测）
> **当前口径**：按"可验收 MVP / 原型验证 / 占位设计 / 未接入生产链路"统计，避免把单元测试通过等同于生产完成。
> **本次更新重点**：Relay Admin API 与 Dashboard 客户端监控进入 Beta 收口，已补齐在线客户端列表、断开入口、审计查询、运行配置状态和部署变量；桌面端新增服务端实例检测，按 Relay、Control Plane、STUN 给出可执行修复建议。本次补充真实生产外部验收清单，Dashboard HTTPS、Windows/macOS 真实 TUN、eBPF 吞吐/延迟基准和真实多地域 Edge/Anycast 均需外部资源满足后复验。
...
| P1 | 发布包实物验收 | 本地质量门禁通过；需生成并在目标平台安装验证 | Git tag `v0.6.0-beta`；Release 构建环境；Windows/macOS/Linux 目标机；代码签名/公证资源（如启用）；SHA256 校验产物；官方 Wintun 来源 | `scripts/package-server.ps1`、`scripts/package-cli.ps1`、`scripts/package-desktop.ps1`、`scripts/package-macos.sh` | server、cli、Windows desktop、macOS DMG 产物带 `v0.6.0-beta` 版本；安装后可启动、连接、查看 Dashboard 状态；校验文件与发布说明一致 |
