...
> **最后更新时间**：2026-06-09（服务端私有 Release 下载路径补齐）
> **当前口径**：按"可验收 MVP / 原型验证 / 占位设计 / 未接入生产链路"统计，避免把单元测试通过等同于生产完成。
> **本次更新重点**：确认 Phase 4 迭代 1/2/3 已完成；服务端一键部署切换为 Release 包安装模式，并补齐 1Panel 部署配置教程；针对私有 GitHub Release 匿名下载 404，补齐 Token/API 下载路径和腾讯云部署说明。
...
| 2026-06-09 | 私有 Release 下载路径验证 | ✅ 通过 | `gh repo view` 确认仓库为 private；匿名 `curl -I -L https://github.com/Lee-zg/NexTunnel/releases/download/v0.0.1-alpha/install.sh` 返回 404；GitHub API 资产地址加 `Authorization: Bearer` 和 `Accept: application/octet-stream` 可下载 `install.sh`，部署脚本已补齐 `NEXTUNNEL_GITHUB_TOKEN/GITHUB_TOKEN` 下载私有服务端包 |
...
| 日期 | 更新内容 | 操作人 |
|:---:|:---|:---:|
| 2026-06-09 | **服务端私有 Release 下载路径补齐**：确认 `raw.githubusercontent.com/Lee-zg/NexTunnel/v0.0.1-alpha/deploy/server/install.sh` 和匿名 Release 附件下载 404 的根因是仓库为 private；`install.sh` 新增 `NEXTUNNEL_GITHUB_TOKEN/GITHUB_TOKEN/--github-token` 支持，未指定自定义包源时自动通过 GitHub API 查找并下载私有 Release 服务端资产；README 增加腾讯云服务器 Token 下载、COS/离线包备用部署命令 | Agent |
