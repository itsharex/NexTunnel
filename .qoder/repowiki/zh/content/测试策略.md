# 测试策略

<cite>
**本文引用的文件**
- [.github/workflows/ci.yml](file://.github/workflows/ci.yml)
- [Makefile](file://Makefile)
- [desktop/go.mod](file://desktop/go.mod)
- [server/go.mod](file://server/go.mod)
- [pkg/go.mod](file://pkg/go.mod)
- [desktop/frontend/package.json](file://desktop/frontend/package.json)
- [desktop/frontend/vite.config.ts](file://desktop/frontend/vite.config.ts)
- [desktop/frontend/eslint.config.js](file://desktop/frontend/eslint.config.js)
- [desktop/internal/auth/token_test.go](file://desktop/internal/auth/token_test.go)
- [desktop/internal/config/store_test.go](file://desktop/internal/config/store_test.go)
- [desktop/internal/tunnel/tunnel_test.go](file://desktop/internal/tunnel/tunnel_test.go)
- [desktop/internal/tunnel/integration_test.go](file://desktop/internal/tunnel/integration_test.go)
- [pkg/protocol/codec_test.go](file://pkg/protocol/codec_test.go)
- [desktop/internal/auth/mtls_test.go](file://desktop/internal/auth/mtls_test.go)
- [pkg/tlsutil/tlsutil_test.go](file://pkg/tlsutil/tlsutil_test.go)
- [pkg/audit/audit_test.go](file://pkg/audit/audit_test.go)
- [pkg/ipam/ipam_test.go](file://pkg/ipam/ipam_test.go)
- [pkg/ipam/route_test.go](file://pkg/ipam/route_test.go)
</cite>

## 目录
1. [引言](#引言)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [新增功能测试策略](#新增功能测试策略)
7. [依赖分析](#依赖分析)
8. [性能考虑](#性能考虑)
9. [故障排查指南](#故障排查指南)
10. [结论](#结论)
11. [附录](#附录)

## 引言
本测试策略文档面向 NexTunnel 项目的开发者与质量保障人员，系统化地定义单元测试、集成测试与端到端测试的实施方法，覆盖网络通信协议、隧道管理、认证令牌、mTLS、审计日志、IPAM等关键模块。文档同时提供测试用例设计原则、测试数据准备与测试环境配置建议，并给出覆盖率目标、持续集成配置与自动化测试流程说明，帮助团队建立稳定、可维护且高效的测试体系。

## 项目结构
NexTunnel 采用多模块组织方式：Go 后端（desktop、server、pkg）与前端（desktop/frontend）。CI 工作流在 GitHub Actions 中统一执行 Go 语言与前端的构建、静态检查与测试；本地通过 Makefile 提供一致化的开发与测试命令。

```mermaid
graph TB
subgraph "桌面应用(Desktop)"
D1["desktop/internal/auth<br/>认证模块"]
D2["desktop/internal/config<br/>配置与SQLite存储"]
D3["desktop/internal/tunnel<br/>隧道管理与集成测试"]
D4["desktop/frontend<br/>Vue 前端"]
end
subgraph "服务端(Server)"
S1["server/cmd/*<br/>控制面/中继/NAT探测二进制"]
end
subgraph "共享库(Pkg)"
P1["pkg/protocol<br/>消息编解码与连接封装"]
P2["pkg/types<br/>通用类型"]
P3["pkg/crypto<br/>加密工具"]
P4["pkg/tlsutil<br/>mTLS证书工具"]
P5["pkg/audit<br/>审计日志"]
P6["pkg/ipam<br/>IP地址管理"]
end
D1 --> P1
D2 --> P1
D3 --> P1
S1 --> P1
S1 --> P4
S1 --> P5
S1 --> P6
```

**图表来源**
- [Makefile:41-52](file://Makefile#L41-L52)
- [desktop/go.mod:1-49](file://desktop/go.mod#L1-L49)
- [server/go.mod:1-11](file://server/go.mod#L1-L11)
- [pkg/go.mod:1-4](file://pkg/go.mod#L1-L4)

**章节来源**
- [Makefile:1-66](file://Makefile#L1-L66)
- [desktop/go.mod:1-49](file://desktop/go.mod#L1-L49)
- [server/go.mod:1-11](file://server/go.mod#L1-L11)
- [pkg/go.mod:1-4](file://pkg/go.mod#L1-L4)

## 核心组件
- 认证模块（desktop/internal/auth）
  - 职责：生成、验证与刷新 JWT 风格令牌，支持过期判断与"即将过期"窗口检测。
  - 关键测试：生成/验证/刷新、过期令牌处理、错误密钥、畸形令牌、唯一性校验。
- 配置模块（desktop/internal/config）
  - 职责：基于 SQLite 的隧道配置与设置项持久化，提供 CRUD、计数、按名称查询与状态更新。
  - 关键测试：打开/关闭数据库、CRUD 与重复名约束、设置项读写、默认路径行为。
- 协议编解码（pkg/protocol）
  - 职责：消息类型、负载编码/解码、连接封装与并发读写、异常输入保护。
  - 关键测试：消息往返、负载解析、截断与超大载荷、空读、并发读写、连接关闭。
- 隧널管理（desktop/internal/tunnel）
  - 职责：客户端隧道生命周期管理、心跳、重连、状态上报与端到端数据通路。
  - 关键测试：TCP 端到端、重连后恢复、多隧道注册顺序、SQLite 持久化、认证令牌生命周期。
- mTLS 模块（desktop/internal/auth + pkg/tlsutil）
  - 职责：双向 TLS 认证、证书生成与加载、客户端证书验证。
  - 关键测试：证书配置验证、mTLS 握手、客户端证书拒绝、错误路径处理。
- 审计日志（pkg/audit）
  - 职责：结构化安全事件记录、JSON 文件与 SQLite 存储、查询过滤。
  - 关键测试：事件创建、日志写入、查询过滤、索引有效性。
- IPAM 模块（pkg/ipam）
  - 职责：虚拟网络 IP 地址分配、路由管理、并发安全。
  - 关键测试：IP 分配回收、网关保留、子网边界、并发访问、路由表操作。

**章节来源**
- [desktop/internal/auth/token_test.go:1-130](file://desktop/internal/auth/token_test.go#L1-L130)
- [desktop/internal/config/store_test.go:1-211](file://desktop/internal/config/store_test.go#L1-L211)
- [pkg/protocol/codec_test.go:1-267](file://pkg/protocol/codec_test.go#L1-L267)
- [desktop/internal/tunnel/tunnel_test.go:1-303](file://desktop/internal/tunnel/tunnel_test.go#L1-L303)
- [desktop/internal/tunnel/integration_test.go:1-535](file://desktop/internal/tunnel/integration_test.go#L1-L535)
- [desktop/internal/auth/mtls_test.go:1-82](file://desktop/internal/auth/mtls_test.go#L1-L82)
- [pkg/tlsutil/tlsutil_test.go:1-360](file://pkg/tlsutil/tlsutil_test.go#L1-L360)
- [pkg/audit/audit_test.go:1-196](file://pkg/audit/audit_test.go#L1-L196)
- [pkg/ipam/ipam_test.go:1-186](file://pkg/ipam/ipam_test.go#L1-L186)
- [pkg/ipam/route_test.go:1-109](file://pkg/ipam/route_test.go#L1-L109)

## 架构总览
下图展示测试金字塔在本项目中的落地：底层是协议与工具层的单元测试，中间是桌面端模块的单元与集成测试，顶层是端到端场景与网络通信测试。

```mermaid
graph TB
subgraph "单元测试(Unit)"
U1["pkg/protocol/codec_test.go"]
U2["desktop/internal/auth/token_test.go"]
U3["desktop/internal/config/store_test.go"]
U4["desktop/internal/auth/mtls_test.go"]
U5["pkg/tlsutil/tlsutil_test.go"]
U6["pkg/audit/audit_test.go"]
U7["pkg/ipam/ipam_test.go"]
U8["pkg/ipam/route_test.go"]
end
subgraph "集成测试(Integration)"
I1["desktop/internal/tunnel/integration_test.go"]
end
subgraph "端到端测试(E2E)"
E1["desktop/internal/tunnel/tunnel_test.go"]
end
U1 --> I1
U2 --> I1
U3 --> I1
U4 --> I1
U5 --> I1
U6 --> I1
U7 --> I1
U8 --> I1
I1 --> E1
```

**图表来源**
- [pkg/protocol/codec_test.go:1-267](file://pkg/protocol/codec_test.go#L1-L267)
- [desktop/internal/auth/token_test.go:1-130](file://desktop/internal/auth/token_test.go#L1-L130)
- [desktop/internal/config/store_test.go:1-211](file://desktop/internal/config/store_test.go#L1-L211)
- [desktop/internal/tunnel/integration_test.go:1-535](file://desktop/internal/tunnel/integration_test.go#L1-L535)
- [desktop/internal/tunnel/tunnel_test.go:1-303](file://desktop/internal/tunnel/tunnel_test.go#L1-L303)
- [desktop/internal/auth/mtls_test.go:1-82](file://desktop/internal/auth/mtls_test.go#L1-L82)
- [pkg/tlsutil/tlsutil_test.go:1-360](file://pkg/tlsutil/tlsutil_test.go#L1-L360)
- [pkg/audit/audit_test.go:1-196](file://pkg/audit/audit_test.go#L1-L196)
- [pkg/ipam/ipam_test.go:1-186](file://pkg/ipam/ipam_test.go#L1-L186)
- [pkg/ipam/route_test.go:1-109](file://pkg/ipam/route_test.go#L1-L109)

## 详细组件分析

### 认证模块测试策略
- 测试目标
  - 令牌生成与验证正确性、过期处理、刷新能力、唯一性保证。
  - 错误场景：错误密钥、畸形令牌、过期令牌。
- 边界条件
  - 过短/过长有效期、即将过期窗口阈值、无效输入。
- 测试用例设计原则
  - 正交覆盖：成功路径、失败路径、边界值。
  - 可重复性：使用固定密钥与时间参数，避免时钟抖动。
  - 可观测性：断言关键字段（如签发/过期时间、客户端ID）。
- 数据准备
  - 固定测试密钥与客户端ID；使用负/正有效期构造过期/未过期场景。
- 性能与并发
  - 无需并发测试，但可扩展为批量生成/验证以评估吞吐。
- 调试技巧
  - 打印令牌字符串与解析后的声明；对比期望与实际字段。
  - 使用子测试分组，便于定位失败点。

```mermaid
sequenceDiagram
participant T as "测试用例"
participant G as "GenerateToken"
participant V as "ValidateToken"
participant R as "RefreshToken"
T->>G : "生成令牌(有效/过期/错误密钥)"
G-->>T : "返回令牌"
T->>V : "验证令牌"
V-->>T : "返回Claims或错误"
T->>R : "刷新过期令牌"
R-->>T : "返回新令牌"
T->>V : "验证刷新后的令牌"
V-->>T : "返回Claims"
```

**图表来源**
- [desktop/internal/auth/token_test.go:12-130](file://desktop/internal/auth/token_test.go#L12-L130)

**章节来源**
- [desktop/internal/auth/token_test.go:1-130](file://desktop/internal/auth/token_test.go#L1-L130)

### 配置模块测试策略
- 测试目标
  - SQLite 打开/关闭、CRUD、按名称查询、计数、状态更新、删除不存在项。
  - 设置项读取/写入/更新、默认路径行为。
- 边界条件
  - 重复名称、删除不存在项、空数据库、默认路径解析。
- 测试用例设计原则
  - 隔离性：每个测试使用独立临时目录与数据库文件。
  - 完整性：覆盖所有公开接口；对错误场景断言。
- 数据准备
  - 临时目录存放数据库文件；使用不同 ID/名称构造冲突与非冲突场景。
- 性能与并发
  - 无需并发测试；关注 I/O 行为与事务一致性。
- 调试技巧
  - 打印数据库路径与最终状态；对比列表长度与计数。

```mermaid
flowchart TD
Start(["开始"]) --> Open["打开数据库"]
Open --> CRUD["创建/读取/更新/删除"]
CRUD --> List["列出与计数"]
List --> Settings["读取/写入设置"]
Settings --> Close["关闭数据库"]
Close --> End(["结束"])
```

**图表来源**
- [desktop/internal/config/store_test.go:30-128](file://desktop/internal/config/store_test.go#L30-L128)

**章节来源**
- [desktop/internal/config/store_test.go:1-211](file://desktop/internal/config/store_test.go#L1-L211)

### 协议编解码测试策略
- 测试目标
  - 消息读写往返、负载解析、异常输入保护（截断头/尾、超大载荷、空读）。
  - 并发读写、连接关闭后的错误传播。
- 边界条件
  - 载荷大小越界、空载荷、零长度读取、并发竞争。
- 测试用例设计原则
  - 结构化用例：覆盖所有消息类型与典型负载。
  - 异常用例：构造非法输入，确保返回预期错误。
  - 并发用例：多 goroutine 交替读写，验证线程安全。
- 数据准备
  - 内存缓冲区模拟网络；net.Pipe 实现本地对等连接。
- 性能与并发
  - 大量消息并发读写，评估吞吐与延迟。
- 调试技巧
  - 打印消息类型与负载；对比期望与实际负载字节序列。

```mermaid
classDiagram
class Message {
+Type
+Payload
+DecodePayload()
}
class Conn {
+Read() Message
+Write(Message)
+Close()
}
Message <.. Conn : "读写"
```

**图表来源**
- [pkg/protocol/codec_test.go:11-267](file://pkg/protocol/codec_test.go#L11-L267)

**章节来源**
- [pkg/protocol/codec_test.go:1-267](file://pkg/protocol/codec_test.go#L1-L267)

### 隧道管理测试策略
- 测试目标
  - 端到端 TCP 传输、重连后恢复、多隧道注册顺序、SQLite 持久化、认证令牌生命周期。
- 测试用例设计原则
  - 场景驱动：从最小可用中继与回显服务器出发，逐步增加复杂度。
  - 时序敏感：等待注册完成、心跳触发、重连窗口。
  - 状态可观测：检查状态报告与流量统计。
- 数据准备
  - 本地回显服务器；自建最小中继（控制/代理端口），模拟工作连接建立。
- 性能与并发
  - 并发数据传输、高 QPS 心跳、重连退避策略验证。
- 调试技巧
  - 开启日志、打印会话ID与状态变化；分阶段断言（连接、注册、传输、断开）。

```mermaid
sequenceDiagram
participant C as "客户端Manager"
participant R as "中继(控制/代理)"
participant E as "回显服务"
participant N as "网络"
C->>R : "认证/心跳/新建代理"
R-->>C : "认证响应/代理响应"
C->>R : "启动工作连接"
R->>N : "转发外部连接"
N->>E : "连接到本地回显"
E-->>N : "回显数据"
N-->>R : "返回数据"
R-->>C : "数据到达"
C-->>C : "状态上报/流量统计"
```

**图表来源**
- [desktop/internal/tunnel/tunnel_test.go:208-302](file://desktop/internal/tunnel/tunnel_test.go#L208-L302)
- [desktop/internal/tunnel/integration_test.go:193-298](file://desktop/internal/tunnel/integration_test.go#L193-L298)

**章节来源**
- [desktop/internal/tunnel/tunnel_test.go:1-303](file://desktop/internal/tunnel/tunnel_test.go#L1-L303)
- [desktop/internal/tunnel/integration_test.go:1-535](file://desktop/internal/tunnel/integration_test.go#L1-L535)

## 新增功能测试策略

### mTLS 测试策略
- 测试目标
  - mTLS 配置验证、证书加载、双向认证握手、客户端证书拒绝。
  - 错误场景：不完整配置、无效证书路径、证书验证失败。
- 边界条件
  - 空配置、部分配置、无效文件路径、过期证书、不受信 CA。
- 测试用例设计原则
  - 证书生成：使用 tlsutil.GenerateSelfSignedCA 和 GenerateSignedCert 创建测试证书。
  - mTLS 握手：通过 httptest 验证客户端和服务端证书交换。
  - 错误处理：验证错误路径的适当返回和错误信息。
- 数据准备
  - 临时目录生成 CA、服务端、客户端证书；使用 WritePEMFiles 写入文件。
  - 通过 LoadServerTLS 和 LoadClientTLS 加载配置。
- 性能与并发
  - mTLS 握手性能基准；证书缓存效果验证。
- 调试技巧
  - 检查证书链完整性；验证 ExtKeyUsage 和 SAN 字段。
  - 使用 RootCAs 和 ClientCAs 验证信任链。

```mermaid
sequenceDiagram
participant CA as "CA证书"
participant SC as "服务端证书"
participant CC as "客户端证书"
participant S as "服务端"
participant C as "客户端"
CA->>SC : "签名服务端证书"
CA->>CC : "签名客户端证书"
S->>C : "TLS握手"
C->>S : "提供客户端证书"
S->>S : "验证客户端证书"
S-->>C : "双向认证成功"
```

**图表来源**
- [desktop/internal/auth/mtls_test.go:38-74](file://desktop/internal/auth/mtls_test.go#L38-L74)
- [pkg/tlsutil/tlsutil_test.go:139-222](file://pkg/tlsutil/tlsutil_test.go#L139-L222)

**章节来源**
- [desktop/internal/auth/mtls_test.go:1-82](file://desktop/internal/auth/mtls_test.go#L1-L82)
- [pkg/tlsutil/tlsutil_test.go:1-360](file://pkg/tlsutil/tlsutil_test.go#L1-L360)

### 审计日志测试策略
- 测试目标
  - 审计事件创建、JSON 文件写入、SQLite 查询、索引有效性。
  - 错误场景：无效路径、查询不支持、数据库连接失败。
- 边界条件
  - 空事件、空详情、空源IP、时间范围边界、结果枚举。
- 测试用例设计原则
  - 事件完整性：验证所有字段正确序列化。
  - 查询过滤：Actor、Action、Resource、时间范围、限制。
  - 存储后端：JSON 文件追加模式、SQLite 索引查询。
- 数据准备
  - 临时文件路径用于 JSON 日志；SQLite 内存数据库用于查询测试。
  - 使用 NewEvent 创建标准审计事件。
- 性能与并发
  - JSON Lines 写入性能；SQLite 查询索引效果。
- 调试技巧
  - 验证 JSON Lines 格式；检查 RFC3339Nano 时间格式。
  - 通过 LIMIT 和 ORDER BY 验证查询排序。

```mermaid
flowchart TD
Start(["审计事件"]) --> Create["创建事件"]
Create --> JSON["JSON文件写入"]
Create --> SQLite["SQLite存储"]
JSON --> Read["读取验证"]
SQLite --> Query["查询过滤"]
Query --> Index["索引使用"]
Read --> End(["完成"])
Index --> End
```

**图表来源**
- [pkg/audit/audit_test.go:33-93](file://pkg/audit/audit_test.go#L33-L93)
- [pkg/audit/audit_test.go:161-176](file://pkg/audit/audit_test.go#L161-L176)

**章节来源**
- [pkg/audit/audit_test.go:1-196](file://pkg/audit/audit_test.go#L1-L196)

### IPAM 测试策略
- 测试目标
  - IP 地址分配与回收、网关保留、子网边界检查、并发安全。
  - 错误场景：无效子网、网关超出子网、IP耗尽、并发冲突。
- 边界条件
  - /30 子网（仅2个可用主机）、网络地址、广播地址、网关地址。
- 测试用例设计原则
  - 分配策略：首次适应算法、循环搜索、重复分配一致性。
  - 并发安全：50个goroutine并发分配、获取、列出。
  - 边界检查：网络地址、广播地址、网关保留、子网包含。
- 数据准备
  - 不同 CIDR：/24、/30、/16；网关在子网内。
  - 并发测试使用 WaitGroup 等待完成。
- 性能与并发
  - 大规模 IP 分配性能；锁竞争分析。
- 调试技巧
  - 验证 IP 在子网范围内；检查 nextHost 循环逻辑。
  - 通过 ListAllocations 验证状态一致性。

```mermaid
flowchart TD
Start(["IPAM初始化"]) --> Parse["解析CIDR和网关"]
Parse --> Validate["验证子网有效性"]
Validate --> Init["初始化分配器"]
Init --> Allocate["分配IP"]
Allocate --> Check["检查保留地址"]
Check --> Store["存储分配结果"]
Store --> Next["更新nextHost"]
Next --> Allocate
```

**图表来源**
- [pkg/ipam/ipam_test.go:36-74](file://pkg/ipam/ipam_test.go#L36-L74)
- [pkg/ipam/ipam_test.go:146-166](file://pkg/ipam/ipam_test.go#L146-L166)

**章节来源**
- [pkg/ipam/ipam_test.go:1-186](file://pkg/ipam/ipam_test.go#L1-L186)
- [pkg/ipam/route_test.go:1-109](file://pkg/ipam/route_test.go#L1-L109)

## 依赖分析
- 模块间依赖
  - desktop 依赖 pkg/protocol 进行网络通信；依赖 sqlite 实现配置持久化。
  - server 依赖 pkg/protocol、pkg/types、pkg/tlsutil、pkg/audit、pkg/ipam 进行控制面交互。
  - 前端依赖 Vue/Pinia，构建由 Vite 管理。
- 测试耦合
  - 隧道测试依赖协议编解码与认证模块；配置测试依赖 SQLite。
  - mTLS 测试依赖 tlsutil；审计日志测试依赖文件系统和 SQLite。
  - IPAM 测试依赖网络库和并发包。
- 外部依赖
  - Go 生态：golangci-lint、wails、websocket、sqlite、crypto/x509。
  - 前端生态：Vue、Vite、ESLint。

```mermaid
graph LR
D["desktop"] --> P["pkg/protocol"]
D --> S["modernc.org/sqlite"]
Srv["server"] --> P
Srv --> T["pkg/tlsutil"]
Srv --> A["pkg/audit"]
Srv --> I["pkg/ipam"]
FE["frontend"] --> V["vue"]
FE --> VT["vite"]
```

**图表来源**
- [desktop/go.mod:5-48](file://desktop/go.mod#L5-L48)
- [server/go.mod:5-10](file://server/go.mod#L5-L10)
- [desktop/frontend/package.json:12-24](file://desktop/frontend/package.json#L12-L24)

**章节来源**
- [desktop/go.mod:1-49](file://desktop/go.mod#L1-L49)
- [server/go.mod:1-11](file://server/go.mod#L1-L11)
- [pkg/go.mod:1-4](file://pkg/go.mod#L1-L4)
- [desktop/frontend/package.json:1-26](file://desktop/frontend/package.json#L1-L26)

## 性能考虑
- 单元测试
  - 协议编解码并发读写测试可作为性能基线；建议记录消息数量与耗时。
  - mTLS 握手性能基准；IPAM 大规模分配性能。
- 集成测试
  - 隧道重连退避、心跳频率与代理端口切换对性能影响显著；应设定上限与回归阈值。
  - 审计日志写入性能；IPAM 并发分配锁竞争。
- 端到端测试
  - 端到端测试应包含吞吐与延迟指标，结合日志统计 BytesIn/BytesOut。
- 基准测试建议
  - 在 pkg/protocol、pkg/tlsutil、pkg/ipam 下引入基准测试，覆盖消息往返、mTLS 握手、IP 分配、审计写入。

## 故障排查指南
- 认证问题
  - 症状：验证失败、过期错误、刷新异常。
  - 排查：确认密钥一致、时间参数合理、令牌未被篡改。
- 配置问题
  - 症状：重复名称、删除失败、默认路径异常。
  - 排查：检查数据库文件路径、事务提交、错误返回码。
- 协议问题
  - 症状：读取截断、超大载荷错误、并发读写失败。
  - 排查：检查消息头长度、负载大小限制、连接状态。
- 隧道问题
  - 症状：无法注册、重连超时、数据不达。
  - 排查：确认中继可达、端口映射、会话ID匹配、状态轮询时机。
- mTLS 问题
  - 症状：握手失败、证书验证错误、客户端证书被拒绝。
  - 排查：检查证书链完整性、SAN 匹配、CA 信任关系。
- 审计日志问题
  - 症状：日志丢失、查询失败、权限错误。
  - 排查：检查文件权限、路径可写性、SQLite 连接。
- IPAM 问题
  - 症状：IP 分配失败、IP 冲突、并发访问错误。
  - 排查：检查子网配置、网关保留、锁机制。

**章节来源**
- [desktop/internal/auth/token_test.go:30-52](file://desktop/internal/auth/token_test.go#L30-L52)
- [desktop/internal/config/store_test.go:153-168](file://desktop/internal/config/store_test.go#L153-L168)
- [pkg/protocol/codec_test.go:117-181](file://pkg/protocol/codec_test.go#L117-L181)
- [desktop/internal/tunnel/integration_test.go:193-298](file://desktop/internal/tunnel/integration_test.go#L193-L298)
- [desktop/internal/auth/mtls_test.go:30-36](file://desktop/internal/auth/mtls_test.go#L30-L36)
- [pkg/audit/audit_test.go:138-143](file://pkg/audit/audit_test.go#L138-L143)
- [pkg/ipam/ipam_test.go:125-144](file://pkg/ipam/ipam_test.go#L125-L144)

## 结论
本测试策略以协议与工具层单元测试为基础，配合桌面端模块的集成测试与端到端场景，形成完整的质量保障闭环。通过明确的测试用例设计原则、边界条件覆盖与调试技巧，团队可以高效发现并修复缺陷。新增的 mTLS、审计日志、IPAM 功能提供了更全面的安全性和可靠性保障。建议在现有基础上补充覆盖率目标与基准测试，持续完善 CI 流程与自动化执行。

## 附录

### 测试用例设计原则
- 完整性：覆盖所有公共接口与错误分支。
- 可重复性：固定种子与输入，避免随机性。
- 可观测性：断言关键状态与指标，输出上下文信息。
- 可维护性：拆分场景、复用辅助函数、清晰命名。

### 测试数据准备
- 临时目录与数据库文件：隔离测试状态。
- 本地回显服务：快速验证数据通路。
- 自建中继：模拟控制/代理通道与会话建立。
- 证书材料：使用 tlsutil 生成测试证书。
- 审计日志：临时文件路径和 SQLite 内存数据库。

### 测试环境配置
- 本地
  - Go 版本：1.23+；Node 版本：20+；Vite 与 ESLint。
  - Makefile 提供统一命令：lint、test、build。
- CI
  - GitHub Actions：分别对 desktop、server、pkg 执行 lint 与测试；前端 lint；构建检查。

**章节来源**
- [Makefile:29-52](file://Makefile#L29-L52)
- [.github/workflows/ci.yml:10-103](file://.github/workflows/ci.yml#L10-L103)
- [desktop/frontend/package.json:6-11](file://desktop/frontend/package.json#L6-L11)
- [desktop/frontend/eslint.config.js:1-16](file://desktop/frontend/eslint.config.js#L1-L16)

### 持续集成与自动化
- 触发条件
  - 推送至 main/develop 分支与拉取请求至 main。
- 步骤概览
  - Go 语言静态检查（golangci-lint）：desktop、server 分别执行。
  - 前端静态检查（ESLint）：desktop/frontend。
  - 构建检查：server 与 pkg 构建；前端构建。
  - Go 测试：desktop、server、pkg 全量测试。
- 建议增强
  - 添加覆盖率收集与阈值告警。
  - 将前端测试纳入 CI（当前脚本存在，可在 CI 中启用）。

**章节来源**
- [.github/workflows/ci.yml:1-103](file://.github/workflows/ci.yml#L1-L103)
- [Makefile:41-52](file://Makefile#L41-L52)