...
## 更新摘要
**所做更改**
- 新增公共发布包下载系统章节，详细说明支持 curl、wget、file 协议的下载方式
- 标准化 Linux shell 脚本换行符，确保在 Linux 服务器上直接执行的兼容性
- 增强 Linux 一键部署流程，完善 systemd 服务管理、权限配置和健康检查机制
- 扩展 Windows PowerShell 部署指南，优化参数传递、进程管理和健康监控
- 移除 GitHub API 认证要求和私有仓库访问依赖，简化部署流程
- 新增自动化权限配置和健康检查机制
...
**更新** 本次更新重点反映了部署流程的重大改进，新增了公共发布包下载方案，标准化了 shell 脚本换行符，增强了 Linux 一键部署流程，并完善了 Windows PowerShell 部署指南。移除了 GitHub API 认证要求和私有仓库访问依赖，大幅简化了部署流程。
