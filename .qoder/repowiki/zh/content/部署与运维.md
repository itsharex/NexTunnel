...
## 更新摘要
**所做更改**
- 移除了私有 GitHub Releases 支持章节，因为服务器部署流程已简化，不再需要 GitHub API 认证
- 移除了 GitHub API 集成章节，新的部署方式直接使用公共 Release 包
- 移除了腾讯云 COS 集成章节，简化了部署流程
- 移除了增强错误处理章节，因为部署流程更加直接
- 更新了企业环境部署指南，现在更加简洁
- 更新了部署脚本功能，移除了 GitHub API 集成和企业环境适配
...
**更新** 本次更新主要反映了服务器部署流程的重大简化，移除了 GitHub API 认证要求和复杂的私有仓库访问流程，新的部署方式更加直接，用户只需使用 curl 或 wget 下载公共 Release 包，无需配置 GitHub Token。
...
### Windows 一键部署脚本（install.ps1）
- 功能特性：支持 install、up、down、restart、status、logs、health、update、uninstall、config 等操作。
- 自动化配置：自动生成 server.env 配置文件，包含随机令牌和端口设置。
- 二进制管理：自动下载 Release 包，校验 SHA256，提取并安装三个核心二进制。
- 进程管理：使用 PID 文件跟踪进程状态，支持优雅启动和停止。
- 健康检查：内置健康检查功能，验证 Control Plane 和 Relay 服务状态。
- 环境集成：支持从 .env 文件、环境变量和交互输入读取配置参数。

**章节来源**
- [deploy/server/install.ps1:1-520](file://deploy/server/install.ps1#L1-L520)

### Linux 一键部署脚本（install.sh）
- systemd 集成：自动创建并管理三个 systemd 服务（nextunnel-relay.service、nextunnel-control-plane.service、nextunnel-nat-detector.service）。
- 权限管理：自动创建系统用户和组，设置适当的文件权限和所有权。
- 配置生成：生成 /etc/nextunnel/server.env 配置文件，支持多种配置方式优先级。
- 简化的包下载：支持从公共 Release 包、HTTP/HTTPS URL 或 file:// 协议下载。
- 健康检查：提供 health 命令检查 Control Plane 和 Relay 服务状态。
- 升级管理：支持 update 操作，无缝升级到新版本。
...
## 结论
本文提供了 NexTunnel 从构建、测试、打包到容器化与 CI/CD 的完整运维蓝图。服务器部署流程的重大简化移除了 GitHub API 认证要求和复杂的私有仓库访问流程，新的部署方式更加直接，用户只需使用 curl 或 wget 下载公共 Release 包，无需配置 GitHub Token。简化的部署脚本支持多种下载方式和非交互部署，大幅提升了部署效率和用户体验。建议在上线前完成端到端联调与压测，并建立完善的日志与告警机制，特别是在企业网络环境下进行充分的网络连通性和性能测试。
