...
subgraph "隧道编排"
E["控制客户端<br/>internal/tunnel/control.go"]
F["隧道管理器<br/>internal/tunnel/manager.go"]
G["单隧道实例<br/>internal/tunnel/tunnel.go"]
end
subgraph "协议与类型"
H["协议消息定义<br/>pkg/protocol/message.go"]
I["协议编解码<br/>pkg/protocol/codec.go"]
J["错误常量<br/>pkg/protocol/errors.go"]
K["共享类型<br/>pkg/types/types.go"]
end
A --> B
B --> C
C --> D
...
## 结论
本文档系统性梳理了 NexTunnel 的桌面端 API 与协议层，明确了 Wails 绑定方法、控制通道交互、消息类型与序列化规则，并提供了前端集成范式、性能与可靠性建议以及故障排查路径。后续版本可在此基础上扩展 UDP 代理与更丰富的安全策略。
