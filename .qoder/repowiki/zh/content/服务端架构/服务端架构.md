...
## 项目结构
服务端采用模块化分层设计，核心位于 server/internal/relay，新增的控制平面、边缘节点管理和SD-WAN组件位于 server/internal 下的相应子目录，协议与共享类型位于 pkg 子模块，桌面端客户端位于 desktop 子目录。
...
subgraph "协议与类型"
P["pkg/protocol/message.go<br/>消息定义与构造"]
Q["pkg/protocol/codec.go<br/>帧格式与编解码"]
R["pkg/protocol/errors.go<br/>协议错误常量"]
S["pkg/types/types.go<br/>共享类型"]
end
subgraph "桌面端"
T["desktop/internal/tunnel/control.go<br/>控制连接客户端"]
U["desktop/internal/tunnel/tunnel.go<br/>工作连接与桥接"]
end
A --> B
G --> H
H --> I
...
## 架构总览
下图展示从客户端到服务端的完整交互链路，包括控制通道认证、代理注册、工作连接建立与数据桥接，以及新增的控制平面协调机制。
...
- **新增依赖**：控制平面依赖节点注册中心、ACL引擎、密钥交换；边缘节点管理依赖注册表；SD-WAN依赖分类器和策略引擎。
...
## 结论
NexTunnel 服务端采用模块化的分布式架构，通过控制平面、边缘节点管理和SD-WAN策略引擎的协同，实现了完整的集群管理、节点注册、负载均衡和高可用部署能力。传统中继服务器保持了简洁高效的事件驱动模型，通过控制通道与工作连接的分工协作，实现了稳定的内网穿透中继能力。新增组件提供了丰富的网络优化和管理功能，包括智能路由、流量整形、安全控制等。协议层以 JSON 负载与自定义帧格式为基础，具备良好的扩展性。当前实现未内置加密，建议在网络边界或前置网关处启用 TLS 以满足生产环境的安全要求。通过合理的配置与监控，可在高并发场景下保持稳定与可观测性。
